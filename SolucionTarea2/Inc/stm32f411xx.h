 ...
 * Datasheer, Memory Map, Figure 14
 ...
 * periféricos descritos en la 1 del manual de referencia del micro.
 * Observer que en dicha tabla está a su vez dividida en cuatro segmentos, cada uno correspondiente
 * a APB1, APB2, AHB1, AHB2.
 *
 * Comenzar de arriba hacia abajo como se muestra en la tabla. Incia USB_OTG_FS (AHB2)
 *
...
/* Posiciones de memoria para periféricos del AMB1
 ...
 * 6.3.1 (RCC_CR) hasta el 6.3.12 (RCC_APB2ENR), 6.3.17 (RCC_BDCR) y (RCC_CSR)
 *
 * Nota: La posición de memoria (offset) debe encajra perfectamente con la posicón de la memoria indicada
 * en la hoja de datos del equipo. Observe que los elementos "reservedx" también están allí presentes.
 **/
 ...
 typedef struct
 {
	 ...
	 volatile uint32_t AHB2LPENR;	//AHB2 clock enable register 					ADDR_OFFSET: 0x54
	 ...
	 volatile uint32_t SSCGR;		//spread spectrum clock generation red			ADDR_OFFSET: 0x80
...
/* 8.4.3 GPIOx_OSPEEDR (dos bit por cada PIN) */
  ...
  * una escritura "atómica", por lo cual es preferible usar el registro BSRR*/
