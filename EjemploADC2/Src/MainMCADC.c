...
EXTEN_Config_t extenConfig				 = {0}; //Configuración del evento externo
...
void initSystem(void) {
	...
	adcConfigEvent.extEventTypeSelect = ADC_EXT_EVENT_EXTI_L11;

	//Se carga la configuración
	adcConfigExternal(&adcConfigEvent);

	//Se configura el evento externo
	extenConfig.pGPIOHandler = &handlerPinTriggerAdc;
	extenConfig.edgeType	 = EXTERNAL_INTERRUPT_FALLING_EDGE;

	configExternalTrigger(&extenConfig);
}
